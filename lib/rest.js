...
var rest = module.exports = {
  ...
  /**
   * 对rest 获取的rest uri进行分发到相应的CRUD
   */
    handleRest: function(req, res) {
        var finalRest, restIf = rest.queryRest(req, res); //restInfo
        req.key = restIf.key;
        this.printf(req, res);
        switch (req.method) {
            case 'GET':
   ...
   * @param req
   * @param res
   * @returns {{res: *, resname: *, key: *, mtd: *}}
   * @desc 因Restful URI 表征资源，相应的URI会影响到collection的设计，因为Restful更应该有好的设计。
