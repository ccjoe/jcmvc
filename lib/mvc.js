...
var mvc = module.exports = {
  ...
  handleMvc: function (req, res) {
    ...
    var view = mvcHandler && mvcHandler.v ? mvcHandler.v : '';
    ...
    if (mvcHandler.a) {
      ...
      if(view){
        ...
        utils.handle404(req, res, 'url【' + req.headers.host+req.url + '】 Not Found!');
      }
    }
    ...
    function render(data) {
      ...
      //渲染模板出错时，直接输出页面(这里的场景是前端渲染时会与服务端渲染冲突, 当服务端渲染出错时，直接输出)
      //@todo 解决为可以服务端与前端共同渲染
      try{
  ...
  /**
   * 根据环境解析前端资源目录的 resource-config-dev.js(开发环境)|resource-config-prod.js(生产环境) 的资源配置
   * @param req
   * @param res
   * @returns {*}
...
module.exports = {
  ...
  parseResource: function (req, res) {
    ...
    //读取配置文件
    try {
      var env = (mvc.config.env || !!~search.indexOf('debug=1')) ? 'dev' : 'prod';
      var resConfig = require(mvc.config.path.fe + 'res-conf-' +env);
    } catch (e) {
      return null;
    }
